...
const AIChat: React.FC = () => {
  ...
  useEffect(() => {
    const handleMusicState = (e: any) => setIsMusicDrawerOpen(e.detail);
    window.addEventListener('music-drawer-state', handleMusicState);
    return () => window.removeEventListener('music-drawer-state', handleMusicState);
  }, []);
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      if (currentScrollY < (isMobile ? 300 : 50)) {
        setIsVisible(!isMobile); // Hide on top for mobile, show for desktop
  ...
  return (
    <>
      ...
      <motion.div
        animate={{ 
          y: (isVisible || isOpen) && !isMusicDrawerOpen ? 0 : 150,
          opacity: (isVisible || isOpen) && !isMusicDrawerOpen ? 1 : 0 
        }}
        transition={{ duration: 0.3 }}
        className={`fixed bottom-4 left-4 sm:bottom-8 sm:left-8 z-[100] ${isMusicDrawerOpen ? 'pointer-events-none' : ''}`}
      >
        <div className="flex flex-col items-center gap-1">
          <motion.button
            ...
            className="p-2 sm:p-3 rounded-full sm:rounded-2xl bg-[#0a192f]/90 backdrop-blur-xl border-2 border-cyan-500/40 hover:border-cyan-400 shadow-2xl group transition-all duration-300 flex items-center justify-center"
          >
          ...
          <span className="sm:hidden text-[8px] font-bold text-cyan-300 uppercase tracking-widest bg-black/40 px-1.5 py-0.5 rounded backdrop-blur-md">
            Hỏi AI
          </span>
        </div>
      </motion.div>
